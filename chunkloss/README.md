...
```text
2 × 8192 × 151674 × 2 bytes ≈ 4.63 GiB
```
...
```text
L = Σ_t CE(h_t W^T, y_t) / α
```

把序列划分为若干互不重叠的块 `C_i` 后：

```text
L = Σ_i [Σ_(t∈C_i) CE(h_t W^T, y_t) / α]
```
...
假设固定 `chunk_size=C`，传统实现沿序列维 `dim=1` 切分：

```text
[B, S, H]
   ├─ [B, C, H] ─ lm_head ─ [B, C, V] ─ CE ─ loss_0
   ├─ [B, C, H] ─ lm_head ─ [B, C, V] ─ CE ─ loss_1
   └─ ...
```
...
传统实现中，`chunk_size` 表示每个样本在序列维上的 token 数；单块 token 总数近似为 `B × chunk_size`。

### 4.2 动态分块

动态模式配置单次计算允许的总 token 上限 `total_chunk_size=T`。运行时根据当前 batch size 计算：

```text
max_chunk = floor(T / B)
chunk_size = 不超过 max_chunk 的最大 2 的幂
```
...
```text
CE_t = logsumexp(logits_t) - correct_class_logit_t
```
...
```text
G = softmax(logits) - one_hot(labels)
dH = G @ W
dW = G^T @ H
```
