...
| 投影 | `lm_head`，权重 $W$ | $W\in\mathbb{R}^{V\times H}$ |
| 输出 | logits | $[B,S,V]$ |
...
多模态大模型常同时具有长序列和大词表。完整 logits 的元素数是 $B \times S \times V$，其显存随序列长度和词表大小线性增长，通常远大于 $[B,S,H]$ 的隐藏状态。交叉熵还可能产生 FP32 logits、softmax 中间量和反向所需状态，因此 loss 阶段会形成明显的显存尖峰；动态 shape 下，反复申请大块张量也容易加剧碎片化。

以仓库单元测试中的一组形状为例：$B=2$、$S=8192$、$V=151674$。仅 BF16 完整 logits 就约为：

$$
2 \times 8192 \times 151674 \times 2\ \text{bytes}
\approx 4.63\ \text{GiB}
$$
...
| 全部 chunk | 按序列维拼接 | 按序列维拼接 | $L=\sum_i L_i$ |

任意时刻只需持有一个 $[B,C,V]$ logits。仅考虑 logits，理想峰值约从 $B \times S \times V$ 降为 $B \times C \times V$，降幅近似为 $S/C$。
...
传统实现中，`chunk_size` 表示每个样本在序列维上的 token 数；若记 $C=\text{chunk\_size}$，单块 token 总数近似为 $B \times C$。

### 4.2 动态分块

动态模式配置单次计算允许的总 token 上限 $T=\text{total\_chunk\_size}$。运行时根据当前 batch size 计算：
...
| Batch size | 理论上限 $\left\lfloor T/B\right\rfloor$ | 实际 `chunk_size` |
|---:|---:|---:|
| 1 | 4096 | 4096 |
| 2 | 2048 | 2048 |
| 3 | 1365 | 1024 |
...
当参数无效，或 $B\ge T$ 时，代码退化为 `chunk_size=1`。动态模式适合 batch size 或输入 shape 经常变化的场景，可让不同 batch 的单块工作量更稳定。
...
代码先完成 next-token label shift：在 labels 右侧补一个 `-100`，再取 `labels[..., 1:]`。这样 hidden position $t$ 对应原始 label $t+1$，最后一个位置被忽略。之后根据 loss 类型计算全局或逐样本归一化系数 $\alpha$，再把 labels 和必要的逐 token $\alpha$ 与 hidden states 按相同边界切块。

关键点是 $\alpha$ 按完整 batch/step 的语义生成，而不是每个 chunk 各自重新统计。因此 chunk 边界不会改变 loss 权重。

## 6. CCE：同时沿词表维流式计算

当前代码还提供 `impl_type: cce`。它源自 Cut Cross Entropy 思路，针对传统 ChunkLoss 仍需生成 $[\text{块内 token 数},V]$ logits 的问题，再沿词表维切成大小为 `vocab_tile_size` 的 tile。

### 6.1 前向

对每个词表区间 `[v0:v1]`：

1. 计算局部 logits：$\mathrm{tile}=H W_{v_0:v_1}^{\mathsf T}$；
...
- 仍然 OOM：逐步减小 `chunk_size`，如 $1024\to512\to256$。
- batch size 经常变化：考虑动态模式，用 `total_chunk_size` 约束 $B\times\text{chunk\_size}$。
