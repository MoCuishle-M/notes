...
设 `𝒜` 是已经扫描的词表项，`ℬ` 是新到来的词表块，且两者不相交。已有状态为 `m_(n,𝒜)`、`s_(n,𝒜)`，新块状态为 `m_(n,ℬ)`、`s_(n,ℬ)`。合并后的最大值为：
...
这里的撇号只表示“合并后得到的新状态”，不是求导符号。这个公式的关键是：`s_(n,𝒜)` 和 `s_(n,ℬ)` 虽然都是移位指数和，但它们分别以 `m_(n,𝒜)` 和 `m_(n,ℬ)` 为基准，不能直接相加。合并前必须先把两个指数和换算到同一个新基准 `m_n'`。
...
等式两边除以 `exp(m_n')`，就得到上面的合并公式。因子 `exp(m_(n,𝒜)-m_n')` 的作用，是把块 `𝒜` 从旧基准 `m_(n,𝒜)` 换算到新基准 `m_n'`；块 `ℬ` 的因子作用相同。

之所以选择两个块最大值中的较大者作为 `m_n'`，是因为此时：

$$
m_{n,\mathcal{A}}-m_n'\leq 0,
\qquad
m_{n,\mathcal{B}}-m_n'\leq 0.
$$
...
合并后的最大值是 `m_n'=4`。块 `𝒜` 原来以 2 为基准，现在要换成以 4 为基准，所以它的贡献需要乘以 `exp(2-4)`；块 `ℬ` 的基准已经是 4，换基准因子为 1：
...
如果错误地直接计算 `s_(n,𝒜)+s_(n,ℬ)`，会得到 `2.735758`，再代入 `m_n'+log(s_n')` 会得到约 `5.006409`，与正确结果不同。错误的原因就是把两个基准不同的移位指数和当成了同一尺度上的量。
...
这恰好是集合 `𝒜∪ℬ` 在新最大值 `m_n'` 下的稳定指数和。因此合并前后的数学值完全一致。
