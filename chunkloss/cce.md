...
a_n=
\begin{cases}
1, & y_n\ne\mathrm{ignore\_index},\\
0, & y_n=\mathrm{ignore\_index}.
\end{cases}
$$
...
CCE 不保存完整的 `z_{n,j}`。它在词表块到来时更新 `\lambda_n` 所需的在线状态，并在正确类别所在的块到来时记录 `c_n`。
...
但当 logit 较大时，直接计算指数容易上溢。稳定做法是减去最大值。对任意有限集合 `\mathcal{A}`，定义：
...
设 `\mathcal{A}` 是已经扫描的词表项，`\mathcal{B}` 是新到来的词表块，且两者不相交。已有状态为 `m_{n,\mathcal{A}}`、`s_{n,\mathcal{A}}`，新块状态为 `m_{n,\mathcal{B}}`、`s_{n,\mathcal{B}}`。合并后的最大值为：
...
为简化书写，令合并后的最大值为 `m_n'`。合并后的移位指数和为：

$$
s_n'
=s_{n,\mathcal{A}}
\exp\left(m_{n,\mathcal{A}}-m_n'\right)
+s_{n,\mathcal{B}}
\exp\left(m_{n,\mathcal{B}}-m_n'\right).
$$
...
这恰好是集合 `\mathcal{A}\cup\mathcal{B}` 在新最大值 `m_n'` 下的稳定指数和。因此合并前后的数学值完全一致。
...
当扫描到第 `k_n` 个词表块时，从局部矩阵 `\mathbf{Z}^{(k_n)}` 读取对应位置即可得到：
...
前向已经保存每个 token 的 `\lambda_n`。反向重新计算第 `k` 个局部 logits tile 后，可以直接恢复这个 tile 内的概率：
...
扫描完全部词表块后，这个累加器恰好等于普通反向的 `\mathbf{G}\mathbf{W}`。
...
不同 tile 写入不重叠的 LM-Head 权重梯度区间，不会遗漏或重复累加。把所有区间按原顺序拼接后，恰好得到普通反向的 `\mathbf{G}^{\mathsf T}\mathbf{X}`。

### 7.5 反向正确性结论

每个 `g_{n,j}` 都通过前向保存的完整词表 `\lambda_n` 精确恢复；所有词表项组成互不相交的完备划分；Transformer 最后一层输出梯度按 tile 求和，LM-Head 权重梯度按 tile 拼接。因此：
