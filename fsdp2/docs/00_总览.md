...
```mermaid
原始 Tensor ──init──▶ sharded DTensor(Shard(0))
                              │ pre-forward: all-gather (AG stream)
                              ▼
                        unsharded Tensor (计算流 forward)
                              │ post-forward: free_storage (无通信)
                              ▼
                        sharded DTensor
                              │ pre-backward: all-gather (AG stream)
                              ▼
                        unsharded Tensor (计算流 backward)
                              │ post-backward: reduce-scatter (RS stream)
                              ▼
                        sharded gradient (写回 sharded_param.grad)
```
